	...
	if (count($errors) == 0) {
		...
		if (mysqli_num_rows($results) == 1) { // user found
			...
			if ($logged_in_user['user_type'] == 'admin') {
				...
				$_SESSION['user_id']=$logged_in_user['userID'];
				$_SESSION['success']  = "You are now logged in";
...
function updatePassword()
{
	...
	$query = "UPDATE user SET password = '$hashedNewPassword' WHERE userID = '$userID'";
	mysqli_query($db, $query);
			...
			if (in_array($fileActualExt, $allowed)) {
				if ($fileError === 0) {
					if ($fileSize < 1000000) {
						...
						$query = "INSERT INTO animal (animalName, animalType, breed, gender, age, vaccinated, dewormed, neutered, animalCondition, imgType) 
					  	VALUES('$animalName', '$animalType','$breed', '$gender', '$age', '$vaccinated', '$dewormed', '$neutered', '$animalCondition', '$fileActualExt')";
						mysqli_query($db, $query);
...
function updateAnimalProfile()
{
	...
	$query = "UPDATE animal SET animalName = '$newName', animalType = '$newType',breed = '$newBreed', gender = '$newGender'
	, age = '$newAge', vaccinated = '$newVaccinated'
